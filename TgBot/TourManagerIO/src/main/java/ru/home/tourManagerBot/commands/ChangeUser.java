...
//если инфа, которую мы получили добавить пользователя и у нас нет еще этого пользователя то
        if (update.getMessage().getText().equals("Редактировать пользователя")){
            //ArrayList<String> status = new ArrayList<>();
            //нулевое значение массива - действие:
            //status.add("Редактировать пользователя");
            ...
            return changeEmail(update);
            //если почта клиента есть в базе, то
        } else if ((BotImplementation.userState.get(userName) != null) && (BotImplementation.userState.get(userName).get(1).equals(update.getMessage().getText())))
        {
            ArrayList<String> status = BotImplementation.userState.get(userName);
            setConfirmationKeyboardMarkup();
            SendMessage sendMessage = new SendMessage();
            sendMessage.setText("Вы хотите редактировать этого пользователя?: "+ update.getMessage().getText());
    ...
    private SendMessage changeEmail(Update update){
        SendMessage sendMessage = new SendMessage();
        sendMessage.setChatId(update.getMessage().getChatId() + "");

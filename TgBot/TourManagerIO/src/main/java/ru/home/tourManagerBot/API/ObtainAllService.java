...
public class ObtainAllService {
    public static UniqueResponse postJSon(ObtainAllClientsRequest request) throws JsonProcessingException {
        UniqueResponse uniqueResponse;

        HttpClient client = HttpClientBuilder.create().build();
        //мне нужен пост запрос, который будет обращаться к адресу в конструкторе
        HttpPost get = new HttpPost("http://localhost:8080/api/client/obtainAll");
